...
void GLCD_flip(void)
{
      ...
      uint8_t t;
      uint8_t p;
      int byteCount=((__GLCD_Screen_Width*__GLCD_Screen_Height/8)/2)-1;
      int bufferEnd = (__GLCD_Screen_Width*__GLCD_Screen_Height/8)-1;
      for (uint16_t i=0; i<=byteCount; i++) {
            ...
            t=__GLCD_Buffer[bufferEnd-i];
            __GLCD_Buffer[bufferEnd-i]=p;
...
static void GLCD_Send(uint8_t Control, uint8_t *Data, uint8_t Length)
{
	Wire.beginTransmission(__GLCD_I2C_Address);
	Wire.write(Control);
